...
def process_message(req: schemas.MessageRequest, request: Request, db: Session = Depends(get_db)):
    ...
    if not req.message or not req.message.strip():
        logger.error(f"[{receive_time}] Empty message received for user_id: {req.user_id}")
        raise HTTPException(status_code=400, detail="Message cannot be empty")
    if not req.user_id or not req.user_id.strip():
        logger.error(f"[{receive_time}] Empty user_id received with message: {req.message}")
    ...
    try:
        ...
        if not kb_context:
            logger.warning(f"[{receive_time}] No suitable answer found in KB for user_id: {req.user_id}")
        ...
        try:
            ...
            logger.error(f"[{receive_time}] LLM/Mock error during reply generation for user_id: {req.user_id}: {e}")
        ...
        if not user:
            user = models.User(user_id=req.user_id, name=req.name, segment=segment)
            db.add(user)
        else:
            user.name = req.name
        ...
        # 6. Log success
        logger.info(f"[{receive_time}] user_id: {req.user_id} | intent: {intent} | segment: {segment} | message: {req.message[:50]}")
        ...
        logger.error(f"[{receive_time}] Unhandled error processing message for user_id: {req.user_id}: {str(e)}")
